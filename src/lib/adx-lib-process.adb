...
function Get_Process_Id return Natural is
   ...
   Process_Id:constant Natural:=Getpid;
...
begin
   ...
   Result:=Adx.Lib.Sysout.Sysout("ps -ax");
...
function Get_Process_List_From_Process_Name(Process_Name:String) return String_Vector_Type is
   ...
   My_Vector:constant String_Vector_Type:=Get_Process_List;
   Result_Vector:String_Vector_Type;
   Process_Name_Escape:constant String:=Adx.Lib.Stringmod.Escape_Regexp(Process_Name);
   Pattern:constant String:="(.*" & Process_Name_Escape & ")";
...
function Get_Process_Id_List_From_Process_Name(Process_Name:String) return String_Vector_Type is
   ...
   My_Vector:constant String_Vector_Type:=Get_Process_List_From_Process_Name(Process_Name);
   Result_Vector:String_Vector_Type;
   Result_Vector2:String_Vector_Type;
   Pattern:constant String:="(.*) pts";
...
function Get_Process_Name_From_Process_Id(Process_Id:Natural) return String is
   ...
   Command:constant String:="ps -p " & Natural'Image(Process_Id) & " -o command=";
   Result:Unbounded_String;

begin

   Result:=Adx.Lib.Sysout.Sysout(Command);
...
procedure Terminate_Process(Process_Id:Natural) is
   ...
   Process_Id_String:constant String:=Adx.Lib.Stringmod.Integer_To_String(Process_Id);
   Command:constant String:="kill -9 " & Process_Id_String;
   Result:Unbounded_String;
